    ...
    <header className="sticky top-0 z-50 slate-nav">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          >
            <div className="relative flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-400 shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform duration-300">
                <span className="text-slate-950 font-black text-sm font-['Outfit'] tracking-tight">SG</span>
              </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden xs:inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping mr-1.5" /> LIVE
                </span>
              </div>
              <p className="text-[10px] text-slate-500 hidden sm:block">India's Verified Price Drop & Loot Alerts</p>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 max-w-md relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-8 py-2 text-sm rounded-xl slate-input placeholder-slate-500 text-white"
            />
            {searchQuery && (
              <button onClick={() => onSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white text-xs font-bold">✕</button>
            )}
          </div>

          {/* Desktop Nav */}
          <nav className="hidden lg:flex items-center gap-0.5 text-xs font-semibold text-slate-500">
            {NAV_ITEMS.map((item) => (
              <button
                key={item.id}
                onClick={item.action}
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-xl bg-emerald-500 text-slate-950 sm:hidden"
            >
              <Send className="w-3.5 h-3.5" />
            </a>
            <button
        ...
        <div className="md:hidden pb-3 pt-1">
          <div className="relative w-full">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => onSearch(e.target.value)}
              className="w-full pl-10 pr-8 py-2 text-xs rounded-xl slate-input placeholder-slate-500 text-white"
            />
            {searchQuery && (
              <button onClick={() => onSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white text-xs font-bold">✕</button>
            )}
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-white/5 bg-[#080c14]/95 backdrop-blur-xl px-4 py-4 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            {NAV_ITEMS.map((item) => (
              <button
                key={item.id}
                onClick={() => { setMobileMenuOpen(false); item.action(); }}
                className="p-2.5 rounded-xl glass-card text-xs font-semibold text-slate-400 text-center hover:text-white transition-colors"
              >
                {item.label}
              </button>
            ))}
          ...
          >
            <Send className="w-3.5 h-3.5" />
            Join Telegram Channel (55K+ Shoppers)
          </a>
        </div>
      )}
    </header>
